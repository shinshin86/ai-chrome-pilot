# ブラウザ自動化ツール比較: このプロジェクト vs Playwright CLI vs Playwright MCP vs Browser Use vs OpenClaw
...
| LLM ループ込みの高レベルな Python 製ブラウザエージェント基盤が欲しい | **Browser Use** |
| エージェント基盤 + サンドボックス + セキュアな運用 | **OpenClaw**                 |

## 概要

AI エージェント（Claude Code 等）からブラウザを操作する主要な5つのアプローチを比較する。
...
| **Browser Use**                                       | browser-use (OSS / Cloud) | Python エージェント実行基盤 + CDP + BrowserSession/watchdogs | Python API / CLI / remote CDP |
...
- **既存タブへのアタッチ不可**: 管理対象のローカル Chrome プロファイルを扱う設計。既に開いているタブへ接続したい場合は Playwright MCP や OpenClaw などの専用ツールが向いている
...
```text
ユーザー → Python アプリ / エージェント → Browser Use の Agent ループ → BrowserSession + Tools + Watchdogs → CDP → Chrome / リモートブラウザ
```

Browser Use は、このドキュメント内の他ツールより一段高い抽象レイヤーにある。単なるブラウザ操作プリミティブの提供ではなく、ブラウザ状態（DOM / アクセシビリティ由来の構造、タブ、スクリーンショット、最近のイベントなど）を収集し、LLM に次の行動を判断させ、構造化アクションを CDP 経由で実行するエージェント実行基盤を含んでいる。

### 主な特徴

- `Agent(...)` 抽象とビルトインのステップ実行ループ
- ローカル起動、既存 CDP 接続、クラウドブラウザ接続を扱える `BrowserSession`
- DOM + アクセシビリティ + スクリーンショットを使ったモデル向けコンテキスト
- click / type / scroll / navigate / upload / evaluate / tabs などのアクションレジストリ
- downloads / popups / screenshots / security / crash などを扱う watchdog ベースのイベント駆動設計
- ブラウザ操作以外も統合できる custom tools

### 優位性

- **エージェントループを内包**: 薄い制御サーバーと違い、Browser Use 自体が reasoning / execution ループを持つ。ブラウザ層と LLM オーケストレーション層を同じランタイムに置きたい場合に向く
- **モデルに渡す状態が豊富**: 単なる snapshot 応答だけでなく、スクリーンショット、DOM 由来の状態、タブ状態、最近のイベントをまとめてモデルに渡せる
- **接続方法が柔軟**: ローカルブラウザ起動、`cdp_url` での既存ブラウザ接続、Browser Use Cloud を単一の抽象で扱える
- **custom tools と深い統合**: Python ベースのエージェント基盤にブラウザ操作と独自ロジックを同居させやすい
- **ガードレールとライフサイクル管理**: ドメイン制限、ポップアップ処理、ダウンロード監視、クラッシュ復旧などがセッション設計に組み込まれている

### 制約

- **重めで意見の強い設計**: これは最小構成のブラウザブリッジではなく、規模の大きい Python 製エージェントフレームワーク。HTTP で `snapshot` / `click` / `type` を明示的に叩きたいだけなら ai-chrome-pilot の方が単純
- **構成要素が多い**: ブラウザセッション管理、プロンプト、アクションモデル、watchdog により高機能だが、その分複雑
- **トランスポート非依存性は低め**: `curl` 中心よりも、Python ライブラリとして組み込むか CLI を使うケースに向く
- **トークン消費は増えやすい**: スクリーンショットや豊富なブラウザ状態を LLM ループに含められるため、最小トークン運用には向かないことがある

### 適したユースケース

- 複数ステップのタスクを自律的に解くブラウザエージェント
- Python ベースのエージェントアプリで、custom tools とブラウザ操作を同じランタイムに載せたい場合
- スクリーンショットや豊富なページ状態、エージェント側のガードレールが欲しいワークフロー
- ローカル / リモート / クラウドブラウザを同じ抽象で扱いたい場合
...
| 機能                         | このプロジェクト |   Playwright CLI    |     Playwright MCP     |                Browser Use                 |              OpenClaw               |
| ---------------------------- | :--------------: | :-----------------: | :--------------------: | :----------------------------------------: | :---------------------------------: |
| ページ遷移                   |        o         |          o          |           o            |                    o                       |                  o                  |
| クリック / テキスト入力      |        o         |          o          |           o            |                    o                       |                  o                  |
| スクリーンショット           |        o         |          o          |           o            |                    o                       |                  o                  |
| ページ構造の自動解析         | o (snapshot + ref ID) |    o (snapshot)     |   o (page_snapshot)    |   o (DOM + AX + screenshot コンテキスト)   |        o (snapshot + ref ID)        |
| 認証状態の保存・復元         | o (プロファイル永続化) | o (state-save/load) | o (persistent profile) |      o (プロファイル / セッション永続)      | o (隔離プロファイル + 手動ログイン) |
| 複数セッション管理           | o (マルチプロファイル) |          o          |           o            |                    o                       |       o (マルチプロファイル)        |
| JavaScript 実行              |    o (/eval)     |          o          |           o            |                o (`eval`)                  |            o (evaluate)             |
| PDF 生成                     |        x         |          o          |           o            |                    x                       |                  o                  |
| ネットワーク監視             |        x         |          x          |           o            |                    x                       |             o (console)             |
| ダイアログ処理               |    o (/dialog)   |          x          |           o            |         o (popup/dialog watchdog)          |             o (dialog)              |
| 既存ブラウザタブ接続         |        x         |          x          |     o (拡張モード)     |   o (`cdp_url` で既存ブラウザへ接続可能)    |        o (Chrome 拡張リレー)        |
| ドラッグ / ホバー / セレクト |    o (/act)      |          x          |           x            | △ (hover/select は組込, drag は custom logic) |                  o                  |
| ファイルアップロード         |        x         |          x          |           x            |                    o                       |                  o                  |
| サンドボックス対応           |        x         |          x          |           x            |                    x                       |                  o                  |
| Playwright なしでの動作      |    o (全操作)    |          x          |           x            |                    o                       |          o (基本操作のみ)           |

### 非機能比較

| 観点               | このプロジェクト | Playwright CLI | Playwright MCP |      Browser Use      |       OpenClaw       |
| ------------------ | :--------------: | :------------: | :------------: | :-------------------: | :------------------: |
| セットアップ容易性 |      ◎ ^1        |       ○        |       △        |        ○ ^5           |   △ (Gateway 必要)   |
| トークン効率       | ○ (ref ベース)   |     ◎ ^2       |       △        |          △            |    ○ (ref ベース)    |
| コード理解の容易さ |      ○ ^3        |   △ (大規模)   |   △ (大規模)   |       △ (大規模)      |      △ (大規模)      |
| カスタマイズ性     |      ◎ ^4        |       ○        |       ○        |          ◎            |          ○           |
...
> ^5 Python + `uv` のセットアップは必要だが、ビルトインのエージェント実行基盤、CLI、リモート / クラウドブラウザ対応を含む
...
Python ファーストなブラウザエージェント基盤が欲しい / LLM オーケストレーション・custom tools・リモートブラウザをまとめて扱いたい
  → Browser Use
